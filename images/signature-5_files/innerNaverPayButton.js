...
naver.NaverPayButton = (function(){
	...
	var NAVER_PAY_BUTTON_SRC_PATTERN = /^(https?:\/\/[^\/]*pay\.naver\.com)\/[^?]*\/innerNaverPayButton.js/;
	...
	NaverPayButton.prepareButtonHTML = function(optionParams) {
		...
		buttonHTML = NaverPayButton.replace(buttonHTML, '${NPAY_CART_LINK_CLASS}', buttonParam.NPAY_TALK_LINK_CLASS);
		
		buttonHTML = NaverPayButton.replace(buttonHTML, '${NPAY_BUY_LINK_STYLE}', buyButtonStyle);
		buttonHTML = NaverPayButton.replace(buttonHTML, '${NPAY_WISH_LINK_STYLE}', wishButtonStyle);
		buttonHTML = NaverPayButton.replace(buttonHTML, '${NPAY_TALK_LINK_STYLE}', talkButtonStyle);
		...
		buttonHTML = NaverPayButton.replace(buttonHTML, '${NPAY_CART_LINK_URL}', optionParams["CART_BUTTON_LINK_URL"]);
	...
	NaverPayButton.applyAllButtonHandler = function(optionParams, forceDisableMessage) {
		...
		if (!forceDisableMessage) {
			...
			if(document.getElementById("NPAY_CART_LINK_ID"+optionParams['BUTTON_ID'])) {
				NaverPayButton.applyButtonHandler(document.getElementById("NPAY_CART_LINK_ID"+optionParams['BUTTON_ID']), optionParams["CART_BUTTON_HANDLER"], optionParams["CART_BUTTON_HANDLER_ARGS"]);
			}
			if(document.getElementById("NPAY_TALK_LINK_ID"+optionParams['BUTTON_ID'])) {
				NaverPayButton.applyButtonHandler(document.getElementById("NPAY_TALK_LINK_ID"+optionParams['BUTTON_ID']), _openTalk, [optionParams['talkAccountId']]);
			}		
		} else {
			if(document.getElementById("NPAY_BUY_LINK_ID"+optionParams['BUTTON_ID'])) {
			...
			if(document.getElementById("NPAY_CART_LINK_ID"+optionParams['BUTTON_ID'])) {
				NaverPayButton.applyButtonHandler(document.getElementById("NPAY_CART_LINK_ID"+optionParams['BUTTON_ID']), function(){alert(forceDisableMessage); return false;});
			}
			if(document.getElementById("NPAY_TALK_LINK_ID"+optionParams['BUTTON_ID'])) {
	...
	NaverPayButton.applyButtonHandler = function(element, handler, argsParam) {
	...
	NaverPayButton.jsonp.callback = function(json) {
		...
		for(var i = 0 ; i < applyButtons.length; i++) {
			...
			if ("PRMT" in json) {
				NaverPayButton.applyPromotion(id, json.PRMT, Buttons[id].PARAM);
			}
		}
	};
	
	var PromotionCheck = {};
	NaverPayButton.applyPromotion = function(buttonIdParam, promotionsParam, buttonParam) {
		...
		function onLoad() {
			...
			if (promotions.length > 1) {
				if (elPromotionPrevBtn && elPromotionNextBtn) {
					NaverPayButton.applyButtonHandler(elPromotionPrevBtn, rollingPrev);
					NaverPayButton.applyButtonHandler(elPromotionNextBtn, rollingNext);
				}
	...
	function _validateGlobalVariable() {
		...
		GLOBALS.LAYOUTHOST = GLOBALS.HOST.replace(NAVER_PAY_URL, LAYOUT_URL);
		return true;
	}
	...
	function _embedStyleSheet() {
		document.write("<link id='"+NAVER_PAY_BUTTON_STYLE_ID+"' type='text/css' rel='stylesheet' href='"+GLOBALS.LAYOUTHOST+"/static/css/button/button2.css?" + Math.round(+new Date()/3600000) + "' />");
	}
